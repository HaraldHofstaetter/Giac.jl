...
diff(ex::giac, vars...) = giac(:derive, Any[ex, vars...])
diff(ex::giac, vars::Array) = giac(:derive, Any[ex, vars])
...
sum(ex::giac, x::giac) = giac(:sum, giac([ex, x], subtype=1))
sum(ex::giac, x::giac, a::Union{giac,Number}, b::Union{giac,Number}) = giac(:sum, giac([ex, x, a, b], subtype=1))
